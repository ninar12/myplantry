...
export function PantryProvider({ children }: { children: ReactNode }) {
  ...
  const markGroceryBought = async (id: string) => {
    ...
    try {
      ...
      const data = await res.json();
      const location = groceryItem.category?.toLowerCase().includes("frozen") ? "freezer"
        : groceryItem.category?.toLowerCase().includes("pantry") || groceryItem.category?.toLowerCase().includes("canned") ? "pantry"
        : "fridge";
      const days =
